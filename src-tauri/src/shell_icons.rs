...
use windows::Win32::UI::WindowsAndMessaging::{
    DestroyIcon, LoadImageW, PostMessageW, SendMessageW, HICON, ICON_BIG, IMAGE_ICON,
    LR_DEFAULTSIZE, LR_LOADFROMFILE, WM_APP, WM_NCDESTROY, WM_SETICON, WM_SETTINGCHANGE,
};

use crate::config::AppState;

const WM_REFRESH_DICTA_ICONS: u32 = WM_APP + 77;
...
unsafe extern "system" fn theme_changed(
    ...
    if message == WM_SETTINGCHANGE || message == WM_REFRESH_DICTA_ICONS {
...
pub fn refresh_after_settings_change(app: &tauri::AppHandle) {
    if let Some(window) = app.get_webview_window("main") {
        if let Ok(handle) = window.hwnd() {
            unsafe {
                let _ = PostMessageW(
                    HWND(handle.0 as isize),
                    WM_REFRESH_DICTA_ICONS,
                    WPARAM(0),
                    LPARAM(0),
                );
            }
