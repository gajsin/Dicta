<script lang="ts">
  ...
  const groupedItems = $derived.by<DateGroup[]>(() => {
    ...
    for (const item of filteredList) {
      ...
      const key = itemMidnight ? String(itemMidnight) : 'invalid-date';
...
<div class="history-container">
  {#if historyList.length === 0}
    <h1 class="history-empty-title">{i18n.t.nav.history}</h1>
  {/if}
  {#if !saveHistory}
  ...
  {:else if historyList.length > 0}
    <div class="history-feed">
      <div class="history-toolbar">
        <h1 class="history-page-title">{i18n.t.nav.history}</h1>
      ...
      {:else}
        ...
        <div class="history-date-group">
          ...
          <div class="history-column-headings" aria-hidden="true">
            <span>{i18n.t.history.columnTime}</span>
            <span>{i18n.t.history.columnText}</span>
            <span>{i18n.t.history.columnDuration}</span>
          </div>
...
<style>
  .history-container {
    ...
    padding: var(--content-top) 24px 0;
  }

  .history-feed {
    --history-row-inset: 10px;
    display: flex;
    flex-direction: column;
    gap: 0;
    width: 100%;
  ...
  .history-page-title { transform: translateY(-5px); }
  ...
  .date-group-header {
    ...
    align-items: center;
    gap: 6px;
    padding-left: var(--history-row-inset);
    user-select: none;
  }
  ...
  .history-column-headings {
    ...
    padding: 0 var(--history-row-inset);
    color: var(--text-tertiary);
    font-size: 12px;
    margin-top: 5px;
    margin-bottom: -5px;
  ...
  .history-column-headings span:last-child {
    text-align: right;
  }
  ...
  .history-item-card:not(:last-child) {
  ...
  :global(.theme-dark) .item-summary-title { color: #D5D8DA; }
