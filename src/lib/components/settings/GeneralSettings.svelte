<script lang="ts">
  ...
  const ACCENT_PRESETS = $derived([
    { color: '#FFC400', name: i18n.t.settings.colorYellow },
...
<div class="settings-page-wrapper">
  <h1 class="page-title">{i18n.t.nav.general}</h1>
  {#if hotkeyErrorMsg}
  ...
  <section class="settings-section">
    ...
    <div class="settings-card">
      ...
      <div class="card-row">
        ...
        <div class="row-controls">
          <span class="history-storage-summary">
            {historyStorageBytes < 1024 ? `${historyStorageBytes} B` : historyStorageBytes < 1048576 ? `${(historyStorageBytes / 1024).toFixed(1)} KB` : `${(historyStorageBytes / 1048576).toFixed(1)} MB`}
            · {i18n.t.history.itemsCount(historyCount)}
          </span>
          <button
            type="button"
...
<style>
  ...
  .settings-section:first-of-type { padding-bottom: 20px; }
  .settings-section:last-of-type { border-bottom: none; }
  .settings-section:first-of-type .card-row { min-height: 34px; }
  .settings-section:nth-of-type(2) { padding-bottom: 16px; }
  .settings-section:nth-of-type(3) { padding-top: 19px; }
  .settings-section:nth-of-type(3) .card-row { min-height: 34px; }
  ...
  .hotkey-recorder-button:focus-visible {
  ...
  .hotkey-recorder-button:hover:not(.capturing) {
    background: var(--hover-surface);
    border-color: var(--border-color);
  }

  :global(.theme-dark) .hotkey-recorder-button:hover:not(.capturing) {
  ...
  .history-clear-action-btn:focus-visible {
  ...
  @media (max-width: 700px) {
    ...
    .settings-section:last-of-type .card-row:last-child {
      align-items: stretch;
      flex-direction: column;
      gap: 8px;
      padding: 8px 0;
    }
    .settings-section:last-of-type .card-row:last-child .row-controls {
      justify-content: space-between;
    }
    .history-storage-summary { width: auto; font-size: 11px; }
    .history-clear-action-btn { width: 136px; }
  }
